...
static lv_res_t confirm_btn_action(lv_obj_t *btn) {
    // Make sure 'roller' and 'textGif' are accessible here
    
    // Get the selected option index from the roller
    selectedAuton = lv_roller_get_selected(roller);
    

    // Clean up the existing gif
    if(textGif) {
        // Ensure that 'clean' method is implemented correctly
        textGif->clean();
    }
    ...
    // Initialize style for the new screen
    static lv_style_t style_new_scr;
    lv_style_copy(&style_new_scr, &lv_style_plain); // Ensure lv_style_plain is initialized

    style_new_scr.body.main_color = LV_COLOR_BLACK;
    style_new_scr.body.grad_color = LV_COLOR_BLACK;

    // Apply the style to the new screen
    // Note: Use lv_obj_add_style() if lv_obj_set_style() is deprecated
    lv_obj_set_style(new_scr, &style_new_scr);

    // Load the new screen
    lv_scr_load(new_scr);

    // Create a container for the GIF
    lv_obj_t *gif_cont = lv_cont_create(new_scr, NULL);
    lv_obj_set_size(gif_cont, LV_HOR_RES / 1.230769230769231, LV_VER_RES / 1);
    lv_obj_align(gif_cont, NULL, LV_ALIGN_CENTER, 0, 0);

    // Check the path and creation of the Gif object
    // Get a random gif string
    std::string randomGif = pickRandomString(matchGifs, matchGifsSize);

    // Make sure the constructor of Gif does not throw an error
    Gif* mainScreenGif = new Gif(randomGif.c_str(), gif_cont);
...
Gif* gifContainer(lv_obj_t *scr) {
    ...
    textGif = new Gif("/usd/343x-text.gif", outer_gif_cont); // The Gif class constructor takes the file path and parent object
    return textGif;
}
...
void runSelectedAuton(AutonFunction autonFunctions[], size_t autonCount) {
    if(selectedAuton >= 0 && selectedAuton < autonCount) {
        ...
        // if we at least have one auton then lets just run that 
        autonFunctions[0].function();
    } else {
        // if we have no autons then do nothing
        doNothing();
    }
}
